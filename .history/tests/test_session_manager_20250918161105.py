...
async def session_manager(mock_browser_cookie3, mock_session_cookies, downloads_path):
    """Create a session manager with mocked dependencies"""
    with patch('src.core.session_manager.InstagramSessionManager._load_cookies_from_browser') as mock_load:
        with patch('src.core.session_manager.InstagramSessionManager._validate_cookies') as mock_validate:
            mock_load.return_value = mock_session_cookies
...
async def test_valid_session(session_manager):
    ...
    with patch('src.core.session_manager.InstagramSessionManager._test_session') as mock_test:
        mock_test.return_value = (True, "Session is valid")
        mock_test.__call__ = AsyncMock(return_value=(True, "Session is valid"))
...
async def test_rate_limited_session(session_manager):
    ...
    with patch('src.core.session_manager.InstagramSessionManager._test_session') as mock_test:
        error = InstagramSessionError("Rate limit exceeded")
        error.is_rate_limit = True
        mock_test.side_effect = error
        mock_test.__call__ = AsyncMock(side_effect=error)
...
async def test_expired_session(session_manager):
    ...
    with patch('src.core.session_manager.InstagramSessionManager._test_session') as mock_test:
        mock_test.return_value = (False, "Session expired")
        mock_test.__call__ = AsyncMock(return_value=(False, "Session expired"))
...
async def test_network_error(session_manager):
    ...
    with patch('src.core.session_manager.InstagramSessionManager._test_session') as mock_test:
        ...
        mock_test.__call__ = AsyncMock(side_effect=Exception("Network error"))
...
@pytest.mark.asyncio
async def test_get_cookies(session_manager, mock_session_cookies):
    """Test getting cookies dictionary"""
    cookies = await session_manager.get_cookies()
    assert cookies == mock_session_cookies

@pytest.mark.asyncio
async def test_cookie_refresh(session_manager):
    ...
    with patch('src.core.session_manager.browser_cookie3') as mock_browser_cookie3:
        ...
        mock_browser_cookie3.firefox.return_value = [
            Mock(name=name, value=value, domain='instagram.com')
            for name, value in new_cookies.items()
        ]
        
        await session_manager.refresh_cookies()
