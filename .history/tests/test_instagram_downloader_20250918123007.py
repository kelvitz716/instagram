...
class TestURLHandling:
    ...
    def test_extract_identifier_from_post_url(self, downloader: InstagramDownloader):
        """Test URL identifier extraction for different Instagram URLs
        
        This test verifies that the identifier extraction works correctly for:
        - Post URLs (format: /p/{identifier})
        - Reel URLs (format: /reels/{identifier})
        - Story URLs (format: /stories/{username}/{identifier})
        """
        test_cases = [
            (TEST_POST_URL, "ABC123"),
            (TEST_REEL_URL, "DEF456"),
            (TEST_STORY_URL, "789012")
        ]
        
        for url, expected_id in test_cases:
            identifier = downloader._extract_identifier(url)
            assert identifier == expected_id, f"Failed to extract identifier from {url}"
