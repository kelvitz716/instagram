...
# This assumes bot.py is in a 'src' subdirectory, and we want to add the parent directory
...
class EnhancedTelegramBot:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.services.instagram_downloader = self.instagram_downloader  # Fixed attribute name
    ...
    def _schedule_cleanup(self) -> None:
        ...
        # The JobQueue is an optional PTB extra; if it's not available (e.g., PTB
        # installed without [job-queue]), `self.bot_app.job_queue` may be None.
        # Guard against that to avoid crashing during startup.
        job_queue = getattr(self.bot_app, 'job_queue', None)
        if job_queue is not None:
            job_queue.run_repeating(scheduled_cleanup, interval=24*60*60)
        else:
            logger.warning("JobQueue not available; periodic cleanup disabled. Install python-telegram-bot[job-queue] to enable it.")
    ...
    async def _initialize_telegram(self) -> None:
        ...
        await self.telethon_client.start(bot_token=self.config.telegram.bot_token)
    ...
    async def handle_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            ...
            "/cleanup - Manually clean up old downloads\n\n"
    ...
    async def handle_cleanup(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /cleanup command to manually trigger cleanup of old directories."""
        from humanize import naturalsize
        
        # Send initial status
        message = await update.message.reply_text("🧹 Starting cleanup process...")
        ...
        try:
            ...
            # Build response
            response = "♻️ **Cleanup Complete:**\n\n"
            
            if dirs_removed > 0:
                response += f"🗑️ Removed: {dirs_removed:,} old directories\n"
                response += f"💾 Freed up: {naturalsize(bytes_freed, binary=True)}\n\n"
            else:
                response += "✨ No old directories to clean up!\n\n"
                
            response += "📊 **Current Storage:**\n"
            response += f"• Used Space: {naturalsize(storage_stats.get('total_size_mb', 0) * 1024 * 1024, binary=True)}\n"
            response += f"• Active Directories: {storage_stats.get('clean_directories', 0):,}\n"
    ...
    async def handle_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        response += "📥 **Downloads:**\n"
        response += f"• Total Attempts: {total:,}\n"
        response += f"• Successful: {successful:,}\n"
        response += f"• Failed: {failed:,}\n"
        response += f"• Success Rate: {success_rate:.1f}%\n\n"

        # Recent Activity
        response += "⏱️ **Recent Activity:**\n"
        response += f"• Last Hour: {stats.get('downloads_last_hour', 0):,} downloads\n"
        response += f"• Last 24h: {stats.get('downloads_last_24h', 0):,} downloads\n"
        response += f"• Last 7 days: {stats.get('downloads_last_7d', 0):,} downloads\n"
        response += f"• This Month: {stats.get('downloads_this_month', 0):,} downloads\n\n"

        # File Statistics
        total_files = stats.get('total_files_downloaded', 0)
        uploaded = stats.get('successful_file_uploads', 0)
        ...
        response += f"• Uploaded Successfully: {uploaded:,}\n"
        response += f"• Failed Uploads: {failed_uploads:,}\n"
        response += f"• Total Data: {naturalsize(total_bytes, binary=True)}\n\n"
        ...
        if content_stats:
            ...
            for content_type, count in sorted(content_stats.items(), key=lambda x: x[1], reverse=True):
                ...
                response += f"• {icon} {content_type.title()}: {count:,} ({percentage:.1f}%)\n"
            response += "\n"

        # Performance Metrics
        if 'bytes_last_hour' in stats and 'bytes_last_24h' in stats:
            response += "� **Performance:**\n"
            response += f"• Recent Transfer: {naturalsize(stats['bytes_last_hour'], binary=True)}/hour\n"
            response += f"• 24h Volume: {naturalsize(stats['bytes_last_24h'], binary=True)}\n"
            avg_file_size = stats.get('avg_file_size', 0)
            response += f"• Avg File Size: {naturalsize(avg_file_size, binary=True)}\n"
            if 'upload_success_rate' in stats:
                response += f"• Upload Success: {stats['upload_success_rate']:.1f}%\n\n"

        # Storage Information
        response += "💾 **Storage:**\n"
        response += f"• Used Space: {naturalsize(storage_stats.get('total_size_mb', 0) * 1024 * 1024, binary=True)}\n"
        response += f"• Total Directories: {storage_stats.get('total_directories', 0):,}\n"
        response += f"• Old Directories: {storage_stats.get('old_directories', 0):,} (ready for cleanup)"
